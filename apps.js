...
function populateDates() {
  ...
  while (currentWeek < currentDate) {
    ...
    // data generation 7 days after, ie sunday
    var dataGenerationDate = new Date(
...
function cleanData(rawData) {
  const cleanData = rawData
    .filter((r) => r.term.length > 2) // skip small words
...
async function fetchData(selectedDate) {
  ...
  const rightCsvData = await d3.csv(
    `./data/${selectedDate}-top-right.csv`,
    d3.autoType
  );
  const leftCsvData = await d3.csv(
    `./data/${selectedDate}-top-left.csv`,
    d3.autoType
  );
...
function renderForWeek(selectedDate, data) {
  ...
  const totalCount =
    leftData.map((r) => r.count).reduce((a, b) => a + b, 0) +
  ...
  const left = leftData
    .filter((d) => leftTerms.includes(d.term))
    .map((d) => ({
      ...d,
      tfnorm: d.count / totalCount,
    }));
  const right = rightData
    .filter((d) => rightTerms.includes(d.term))
    .map((d) => ({
      ...d,
      tfnorm: d.count / totalCount,
    }));
  const both = bothTerms
    .map((t) => {
      const leftItem = leftData.find((i) => i.term == t);
      const rightItem = rightData.find((i) => i.term == t);
      return {
        term: t,
        count: leftItem.count + rightItem.count,
        tfnorm: (leftItem.count + rightItem.count) / totalCount,
      };
    })
  ...
  const rightSVG = d3
    .select("#right-only-terms")
    .append("svg")
    .attr("width", totalWidth / 3)
    .attr("height", config.height);

  const leftSVG = d3
    .select("#left-only-terms")
    .append("svg")
    .attr("width", totalWidth / 3)
    .attr("height", config.height);
  ...
  leftSVG
    ...
    .node()
    .appendChild(
      orderedWordCloud(totalWidth / 3, left, "#333399", extent, "left-top")
    );
  sharedSVG
    .append("g") // both
    .attr("transform", "translate(0,35)")
    .node()
    .appendChild(
      orderedWordCloud(totalWidth / 3, both, "#800080", extent, "both-top")
    );
  rightSVG
    .append("g") // right
    .attr("transform", "translate(0,35)")
    .node()
    .appendChild(
      orderedWordCloud(totalWidth / 3, right, "#993333", extent, "right-top")
    );

  // combine all three SVG elements into a single selection
  const allSVGs = d3.selectAll([
    leftSVG.node(),
    sharedSVG.node(),
    rightSVG.node(),
  ]);
  ...
  terms
    ...
    .on("mouseout", function () {
      d3.select(this).style("cursor", "default").style("font-weight", "bold");
    })
...
function generateAttentionVisualization(selectedTerm, selectedDate) {
  ...
  function wmAttentionQuery(term, domains, startDateStr, endDateStr) {
    ...
    const params = {
      q: `"${term}" AND ${dateClause} AND ${domainClause}`,
    };
  ...
  async function queryWaybackAPI(
    selectedTerm,
    leftLeaningDomains,
    rightLeaningDomains,
    selectedDate
    ...
    try {
      ...
      // left domain queries
      const leftTermFetches = selectedTermArray.map((term) =>
        wmAttentionQuery(term, leftLeaningDomains, startDateStr, endDateStr)
      );
      const rightTermFetches = selectedTermArray.map((term) =>
        wmAttentionQuery(term, rightLeaningDomains, startDateStr, endDateStr)
      );
      ...
      var promises = Object.entries(weekWordCounts).map(function (
        [word, counts],
        index
      ) {
  ...
  d3.json(filePath).then(function (data) {
    ...
    var allDates = Array.from(
      new Set(data.flatMap((d) => d.data.map((datum) => datum.date)))
    );
    ...
    svg_attention
      .append("g")
      .call(d3.axisLeft(y).ticks(8).tickFormat(d3.timeFormat("%m-%d-%Y")))
      .selectAll("text")
    ...
    var horizontalLine = svg_attention
      .append("line")
    ...
    // if mouse enters the chart area, show the annotations
    function mouseover() {
    ...
    // if mouse moves within the chart area, update the annotations based on mouse position
    function mousemove(event) {
      // Recover mouse coordinates
      var mouseY = d3.pointer(event)[1];
      var y0 = y.invert(mouseY);

      // find corresponding data points for the y-coordinate
      ...
      // find data point closest to the mouse
      var closestData = selectedData.reduce(function (prev, curr) {
        return Math.abs(y(curr.ratio) - mouseY) <
          Math.abs(y(prev.ratio) - mouseY)
          ? curr
          : prev;
      });
      ...
      // update position of the focus box
      ...
      // update  position and content of the focus text
      focusText.attr("x", fbX + 10).attr("y", fbY + 25);
      focusText
        .select("tspan.date")
        .attr("x", fbX + 10)
        .attr("y", fbY + 25)
        .text("Date: " + d3.timeFormat("%m-%d-%Y")(closestData.date));
      focusText
        .select("tspan.ratio")
        .attr("x", fbX + 10)
        .attr("y", fbY + 45)
        .text("Ratio: " + closestData.ratio.toFixed(4));
      ...
      // update position of the horizontal line
      horizontalLine
        .attr("x1", x(-4))
    ...
    // when mouse leaves chart area, hide annotations
    function mouseout() {
...
function listCloudLayout(wordNodes, width, extent, sizeRange) {
  ...
  wordNodes.attr("x", (d) => {
    ...
    let lastX = x;
    if (x + textLength + 10 > width) {
      // TODO: replace 10 with state property for padding
      lastX = 0;
    }
...
function orderedWordCloud(theWidth, data, termColor, exent, id) {
  ...
  while (y >= wordListHeight && sizeRange.max > sizeRange.min) {
    wordNodes = wordWrapper
      .selectAll("text") // one text per term
      .data(data, (d) => d.term)
      .enter()
    ...
    // Layout
    y = 0;
    const leftHeight = listCloudLayout(
      wordNodes,
      innerWidth,
      fullExtent,
      sizeRange
